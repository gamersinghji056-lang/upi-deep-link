            ...
            try {
                ...
                if (exactEvent != null) {
                    val payload = JSONObject()
                        ...
                        .put("utr", exactEvent.utr)
                        .put("amount", exactEvent.amount)
                        .put("sender", exactEvent.sender)
                        .put("receivedAt", Instant.ofEpochMilli(exactEvent.receivedAt).toString())
                    ...
                    if (response.optBoolean("matched")) {
                        store.saveLastEvent("Payment verified: UTR ${exactEvent.utr.takeLast(6)}, INR ${"%.2f".format(exactEvent.amount)}")
                    } else {
                        store.saveLastEvent("Credit received: UTR ${exactEvent.utr.takeLast(6)}, INR ${"%.2f".format(exactEvent.amount)}. Waiting for matching customer UTR.")
                    }
                } else if (reviewCandidate != null) {
                    val payload = JSONObject()
                        .put("simFingerprint", sim.fingerprint)
                        .put("referenceCandidate", reviewCandidate.referenceCandidate)
                        .put("amount", reviewCandidate.amount)
                        .put("sender", reviewCandidate.sender)
                        .put("receivedAt", Instant.ofEpochMilli(reviewCandidate.receivedAt).toString())
                    ApiClient.creditSmsCandidate(store, payload)
                    store.saveLastEvent(
                        "Credit SMS needs review: ${reviewCandidate.referenceCandidate.length}-digit reference, INR ${"%.2f".format(reviewCandidate.amount)}. Auto-success was not allowed."
                    )
                }
