...
async function initDeviceCreditTables(pool) {
  if (!pool) return;
  await pool.query(`
    create table if not exists device_credit_candidates (
      id bigserial primary key,
      device_id text not null references devices(id) on delete cascade,
      reference_digits text not null,
  ...
  `);
  await pool.query("create index if not exists device_credit_candidates_review_idx on device_credit_candidates(review_status, sms_received_at desc)");
}

function normalizeCandidateReference(value) {
  const digits = String(value || "").replace(/\D/g, "");
  if (digits.length < 10 || digits.length > 14 || digits.length === 12) {
    throw new Error("Ambiguous UPI reference must contain 10-14 digits and must not already be a valid 12-digit UTR");
  }
  return digits;
}
...
function createDeviceCreditRouter({ pool }) {
  ...
  // Ambiguous references (for example 11 or 13 digits) are retained for review only.
  // They can never mark a payment successful automatically.
  router.post("/credit-sms-candidate", requireDevice, async (req, res, next) => {
    try {
      if (!requireSimBinding(req, res)) return;
      const referenceDigits = normalizeCandidateReference(req.body?.referenceCandidate);
      const amount = normalizeAmount(req.body?.amount);
      const sender = String(req.body?.sender || "").trim().slice(0, 80) || null;
      const received = req.body?.receivedAt && !Number.isNaN(new Date(req.body.receivedAt).getTime())
        ? new Date(req.body.receivedAt)
        : new Date();

      await pool.query(
        `insert into device_credit_candidates(device_id, reference_digits, digit_count, amount, sender, sms_received_at)
         values($1,$2,$3,$4,$5,$6)`,
        [req.device.id, referenceDigits, referenceDigits.length, amount, sender, received.toISOString()]
      );
      await pool.query("update devices set last_seen_at=now() where id=$1", [req.device.id]);
      await pool.query(
        `insert into device_transactions(device_id, payment_id, utr, status, app, amount, raw_result)
         values($1,null,$2,'REVIEW_REQUIRED','BANK_SMS',$3,$4::jsonb)`,
        [req.device.id, referenceDigits, amount, JSON.stringify({ sender, receivedAt: received.toISOString(), digitCount: referenceDigits.length })]
      );
      ...
      res.status(202).json({
        ...
        matched: false,
        status: "needs_review",
        reason: "UPI reference was not exactly 12 digits; automatic payment success is disabled for ambiguous references."
      });
    } catch (error) {
      if (/reference|amount/i.test(error.message || "")) return res.status(400).json({ error: error.message });
      next(error);
    }
  });
