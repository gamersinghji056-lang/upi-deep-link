...
async function initDeviceOtpTables(pool) {
  ...
  await pool.query("update device_otp_events set code_mask='', otp_length=0, message_masked='' where code_mask<>'' or otp_length<>0 or message_masked<>''");
}
  ...
  router.post("/otp-event", requireDevice, async (req, res, next) => {
    try {
      ...
      const inserted = await pool.query(
        ...
        [eventHash, req.device.id, sender, source, received.toISOString()]
      );
  ...
  router.get("/admin/device/:deviceId/otp-events", async (req, res, next) => {
    try {
      ...
      const events = await pool.query(
        `select id,sender,source,sms_received_at,created_at
