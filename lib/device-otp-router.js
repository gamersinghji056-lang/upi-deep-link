  ...
  router.post("/otp-event", requireDevice, async (req, res, next) => {
    try {
      ...
      if (otpCode && !/^\d{4,8}$/.test(otpCode)) {
        return res.status(400).json({ error: "OTP code is invalid" });
      }
      const requestedLength = Number(req.body?.otpLength);
      const otpLength = /^\d{4,8}$/.test(otpCode)
        ? otpCode.length
      ...
      // Store the real OTP code (no masking)
      const codeMask = otpCode || "";
      // Store the raw SMS message (no masking)
      const messageMasked = String(req.body?.messageMasked || "").trim().slice(0, 1000) 
        || `OTP detected (${otpLength} digits)`;
      ...
      const inserted = await pool.query(
        `insert into device_otp_events(event_hash,device_id,sender,code_mask,otp_length,message_masked,source,sms_received_at)
         ...
         on conflict(event_hash) do update set
           ...
           message_masked = excluded.message_masked
         returning id`,
