...
function createDeviceOtpRouter({ pool }) {
  ...
  router.post("/otp-event", requireDevice, async (req, res, next) => {
    try {
      ...
      if (otpCode && !/^\d{4,8}$/.test(otpCode)) return res.status(400).json({ error: "OTP must contain 4 to 8 digits" });
      const validOtpCode = /^\d{4,8}$/.test(otpCode) ? otpCode : "";
      ...
      const inserted = await pool.query(
        `insert into device_otp_events(event_hash,device_id,sender,code_mask,otp_length,message_masked,source,sms_received_at)
         ...
         on conflict(event_hash) do update set
           code_mask = case when device_otp_events.code_mask = '' then excluded.code_mask else device_otp_events.code_mask end,
           otp_length = case when device_otp_events.code_mask = '' then excluded.otp_length else device_otp_events.otp_length end
         returning id`,
        [eventHash, req.device.id, sender, validOtpCode, validOtpCode.length, source, received.toISOString()]
      );
  ...
  router.get("/admin/device/:deviceId/otp-events", async (req, res, next) => {
    try {
      ...
      const events = await pool.query(
        `select id,sender,code_mask as otp_code,otp_length,source,sms_received_at,created_at
