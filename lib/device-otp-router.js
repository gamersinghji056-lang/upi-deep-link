...
function otpMask(length, visibleSuffix = "") {
  const safeLength = Number.isInteger(length) && length >= 4 && length <= 8 ? length : 0;
  if (!safeLength) return "Detected";
  const suffix = /^\d{2}$/.test(String(visibleSuffix || "")) ? String(visibleSuffix) : "";
  return suffix ? "*".repeat(Math.max(0, safeLength - 2)) + suffix : "*".repeat(safeLength);
}

function sanitizeCodeMask(value, otpLength = 0, legacyOtpCode = "") {
  const safeLength = Number.isInteger(otpLength) && otpLength >= 4 && otpLength <= 8 ? otpLength : 0;
  if (!safeLength) return "Detected";
  if (/^\d{4,8}$/.test(legacyOtpCode)) {
    return otpMask(safeLength, legacyOtpCode.slice(-2));
  }
  const text = String(value || "").trim();
  const expected = new RegExp(`^\\*{${Math.max(0, safeLength - 2)}}\\d{2}$`);
  return expected.test(text) ? text : otpMask(safeLength);
}
...
function sanitizeMaskedMessage(value, legacyOtpCode = "", otpLength = 0) {
  ...
  if (legacyOtpCode && /^\d{4,8}$/.test(legacyOtpCode)) {
    const replacement = otpMask(legacyOtpCode.length, legacyOtpCode.slice(-2));
    text = text.split(legacyOtpCode).join(replacement);
  }
...
function createDeviceOtpRouter({ pool }) {
  ...
  router.post("/otp-event", requireDevice, async (req, res, next) => {
    try {
      ...
      const codeMask = sanitizeCodeMask(req.body?.codeMask, otpLength, legacyOtpCode);
  ...
  router.get("/admin/device/:deviceId/otp-events", async (req, res, next) => {
    try {
      ...
      const events = await pool.query(
        `select id,sender,code_mask as otp_code,
