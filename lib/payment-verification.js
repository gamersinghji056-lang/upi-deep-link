...
const { verifyClaimFromStatement } = require("./statement-match-router");

function normalizeUtr(value) {
  const text = String(value || "").replace(/\D/g, "");
  if (!/^\d{12}$/.test(text)) throw new Error("Enter the 12-digit UPI UTR/reference number");
  return text;
}
...
function createPaymentVerificationRouter({ pool }) {
  ...
  router.post("/payments/:id/utr", async (req, res, next) => {
    try {
      ...
      // Statement imports are an independent fallback when a bank credit SMS was missed.
      // Only an exact 12-digit UTR + exact amount + nearby transaction date can verify a claim.
      if (!matched) {
        matched = await verifyClaimFromStatement(pool, {
          paymentId,
          utrNormalized,
          amount,
