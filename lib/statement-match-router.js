...
async function matchImportedTransactions(pool, rows) {
  const results = [];
  for (const row of rows) {
    ...
    if (!candidates.rowCount) {
      results.push({ ...row, status: "unmatched", paymentId: null });
      continue;
    }
    ...
    results.push({ ...row, status: matched ? "matched" : "unmatched", paymentId: matched ? paymentId : null });
  }
  return results;
}
...
function createStatementMatchRouter({ pool }) {
  ...
  router.post("/match", async (req, res, next) => {
    try {
      ...
      const results = await matchImportedTransactions(pool, rows);
