(function () {
  ...
  $("qrFile").addEventListener("change", async () => {
    ...
    try {
      ...
      $("qrMsg").textContent = "UPI QR loaded. PhonePe mode will preserve the QR and only add missing amount/currency.";
    } catch (error) {
  ...
  $("form").addEventListener("submit", async event => {
    ...
    try {
      ...
      $("debug").textContent = "Short payment ID: " + data.id + "\nApp: PhonePe only\nProfile: standard minimal\nMissing am/cu are added; all existing QR fields are preserved.\nExpires: " + data.expiresIn;
      $("empty").style.display = "none";
      $("result").style.display = "block";
      $("msg").className = "msg ok";
      $("msg").textContent = "PhonePe payment link created.";
    } catch (error) {
      $("msg").className = "msg err";
      $("msg").textContent = error.message;
    } finally {
      $("createBtn").disabled = false;
      $("createBtn").textContent = "Create PhonePe Payment Link";
    }
  });
