(function (root, factory) {
  ...
  function androidIntent(raw, app) {
    ...
    // Avoid interpreting a QR fragment as Android Intent extras or silently deleting it.
    if (parsed.fragment || !raw.startsWith("upi://pay?")) return null;
    const pkg = packages[app];
    return "intent:" + raw.slice(4) + "#Intent;scheme=upi;package=" + pkg + ";S.browser_fallback_url=" + encodeURIComponent("https://play.google.com/store/apps/details?id=" + pkg) + ";end";
  }
  function iosUri(raw, app) {
    const parsed = parse(raw);
    if (parsed.fragment || !raw.startsWith("upi://pay?")) return null;
    const prefix = { gpay: "gpay://upi/pay?", phonepe: "phonepe://pay?", paytm: "paytmmp://pay?" }[app];
    return prefix ? prefix + raw.slice(raw.indexOf("?") + 1) : raw;
  }
  function targets(raw) {
    parse(raw);
    return { generic: raw, android: Object.fromEntries(Object.keys(packages).map(app => [app, androidIntent(raw, app)])), ios: Object.fromEntries(Object.keys(packages).map(app => [app, iosUri(raw, app)])) };
  }
  ...
  return { parse, build, mode, amount, manual, targets, target, packages };
});
