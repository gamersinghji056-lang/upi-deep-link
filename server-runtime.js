...
async function start() {
  ...
  try {
    ...
    app.use((error, _req, res, _next) => {
      ...
      if (status >= 500) console.error("Runtime request failed", error.code || error.name, error.message || "");
      res.status(status).json({ error: status === 500 ? "Could not process request" : (error.message || "Request failed") });
    });

    const host = process.env.NODE_ENV === "development" ? "127.0.0.1" : "0.0.0.0";
    const port = process.env.PORT || 3000;
    return app.listen(port, host, () => console.log("WPAY runtime listening on port " + port));
  } catch (error) {
    await pool?.end();
    console.error("Runtime initialization failed:", !pool ? "DATABASE_URL is missing" : (error.code || error.name), error.message || "");
    process.exitCode = 1;
  }
}
